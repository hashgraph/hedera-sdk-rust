...
#[tokio::test]
async fn remove_allowance_fails() -> anyhow::Result<()> {
    let Some(TestEnvironment { config, client }) = setup_nonfree() else {
        return Ok(());
    };
    ...
    _ = AccountAllowanceApproveTransaction::new()
        .approve_token_allowance(ft.id, receiver_account.id, spender_account_id, 10)
        .freeze_with(&client)?
        .sign(receiver_account.key.clone())
        .execute(&client)
        .await?
    ...
    _ = TransferTransaction::new()
        .approved_token_transfer(ft.id, receiver_account.id, -5)
        .token_transfer(ft.id, spender_account_id, 5)
        .transaction_id(TransactionId::generate(spender_account_id))
        .freeze_with(&client)?
        .sign(spender_account_key.clone())
        .execute(&client)
        .await?
        .get_receipt(&client)
        .await?;
    ...
    assert_matches!(
        res,
        Err(hedera::Error::ReceiptStatus { status: Status::SpenderDoesNotHaveAllowance, .. })
    );
    ...
    _ = TransferTransaction::new()
        .approved_nft_transfer(nft.id.nft(nft_serials[0]), receiver_account.id, spender_account_id)
        .transaction_id(TransactionId::generate(spender_account_id))
        .freeze_with(&client)?
        .sign(spender_account_key.clone())
        .execute(&client)
    ...
    let res = TransferTransaction::new()
        .approved_nft_transfer(nft.id.nft(nft_serials[1]), receiver_account.id, spender_account_id)
        .approved_nft_transfer(nft.id.nft(nft_serials[2]), receiver_account.id, spender_account_id)
        .transaction_id(TransactionId::generate(spender_account_id))
        .freeze_with(&client)?
        .sign(spender_account_key.clone())
        .execute(&client)
