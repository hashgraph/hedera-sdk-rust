    ...
    async throws -> E.Response
{
    ...
    while true {
        ...
        let nodeIndexIndexes = randomIndexes(upTo: nodeIndexes.count, amount: nodeSampleAmount)

        inner: for index in nodeIndexIndexes {
            let nodeIndex = nodeIndexes[index]
            let (nodeAccountId, channel) = client.network.channel(for: nodeIndex, on: client.eventLoop)
