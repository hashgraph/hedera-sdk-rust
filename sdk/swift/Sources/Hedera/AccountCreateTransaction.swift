...
public class AccountCreateTransaction: Transaction {
    ...
    public override func encode(to encoder: Encoder) throws {
        ...
        var accountCreate = container.nestedContainer(keyedBy: AccountCreateKeys.self, forKey: .accountCreate)

        try accountCreate.encode(key, forKey: .key)
        try accountCreate.encode(initialBalance, forKey: .initialBalance)
        try accountCreate.encode(accountMemo, forKey: .accountMemo)

        if autoRenewPeriod != nil {
            try accountCreate.encode(autoRenewPeriod, forKey: .autoRenewPeriod)
        }

        try accountCreate.encode(maxAutomaticTokenAssociations, forKey: .maxAutomaticTokenAssociations)
        try accountCreate.encode(stakedAccountId, forKey: .stakedAccountId)
        try accountCreate.encode(declineStakingReward, forKey: .declineStakingReward)
...
private enum AccountCreateKeys: String, CodingKey {
    case key
    case initialBalance
    case accountMemo
