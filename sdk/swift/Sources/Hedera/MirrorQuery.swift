...
extension MirrorRequest {
    typealias ConnectStream = GRPCAsyncResponseStream<Item.Protobuf>
    typealias ItemStream = AnyAsyncSequence<Item>
...
extension MirrorQuery where Self: MirrorRequest {
    /// Execute this mirror query in a streaming fashion.
    ///
    /// - Returns: some async sequence of ``Item``.
    internal func subscribeInner(_ client: Client, _ timeout: TimeInterval? = nil) -> ItemStream {
        ...
        let channel = client.mirrorNetwork.channel(client.eventLoop)

        return mirrorSubscribe(channel, self, timeout).map(Item.fromProtobuf).eraseToAnyAsyncSequence()
    }
