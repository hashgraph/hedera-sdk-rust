...
internal final class Network: ExpressibleByDictionaryLiteral {
    private init(
        map: [AccountId: Int],
        nodes: [AccountId],
        addresses: [[String]],
    ...
    internal convenience init(dictionaryLiteral elements: (AccountId, [String])...) {
        ...
        self.init(map: map, nodes: nodes, addresses: addresses, channels: channels, healthy: healthy)
    }

    internal func channel(for nodeIndex: Int, on eventLoop: NIOCore.EventLoopGroup) -> (AccountId, GRPCChannel) {
        let accountId = nodes[nodeIndex]
        if let channel = channels[nodeIndex] {
            return (accountId, channel)
        }

        // todo: someone verify this code pls.
        let channel = ChannelBalancer(eventLoop: eventLoop.next(), addresses[nodeIndex].map { .hostAndPort($0, 50211) })

        channels[nodeIndex] = channel
    ...
    internal func healthyNodeIndexes() -> [Int] {
        let now = Int64(Timestamp(from: Date()).unixTimestampNanos)
